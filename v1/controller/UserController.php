<?php
    ...
    if (empty($_GET)) {
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $query = $db->prepare('SELECT users.id, users.fullname, users.email 
                                   FROM '.DatabaseConfig::USERS_TABLE.' users, '.DatabaseConfig::SESSIONS_TABLE.' sessions 
                                   WHERE sessions.userid = users.id AND sessions.accesstoken = :accesstoken');
            ...
            if ($rowCount === 0) {
                ...
                $response->setSuccess(false);
                $response->addMessage("Failed to get a user by providing access token");
                $response->send();
                exit;
            }
